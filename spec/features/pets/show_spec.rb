...
RSpec.describe 'the shelter show' do
  it "shows the shelter and all it's attributes" do
    shelter = Shelter.find_or_create_by!(name: 'Mystery Building', city: 'Irvine CA', foster_program: false, rank: 9)
  ...
  it "allows the user to delete a pet" do
    shelter = Shelter.find_or_create_by!(name: 'Mystery Building', city: 'Irvine CA', foster_program: false, rank: 9)
    pet = Pet.find_or_create_by!(name: 'Scrappy', age: 1, breed: 'Great Dane', adoptable: true, shelter_id: shelter.id)
